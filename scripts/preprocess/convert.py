...
def init_colmap(args):
    ...
    for scene in scene_list:
        ...
        feat_extracton_cmd = (
            ...
            f"--database_path {scene_path}/database.db "
            f"--image_path {scene_path}/input "
        ...
        os.makedirs(os.path.join(scene_path, "dense"), exist_ok=True)
        ### Bundle adjustment
        ...
        # 新增稠密重建步骤
        ### Dense reconstruction steps
        # 使用patch_match_stereo生成深度图
        dense_stereo_cmd = f"{colmap_command} patch_match_stereo \
            --workspace_path {scene_path} \
            --workspace_format COLMAP \
            --PatchMatchStereo.geom_consistency true"
        exit_code = os.system(dense_stereo_cmd)
        if exit_code != 0:
            logging.error(f"Patch match stereo failed with code {exit_code}. Exiting.")
            exit(exit_code)


        # 使用stereo_fusion融合生成稠密点云
        dense_fusion_cmd = f"{colmap_command} stereo_fusion \
            --workspace_path {scene_path}\
            --workspace_format COLMAP \
            --input_type geometric \
            --output_path {scene_path}/dense/fused.ply"
        exit_code = os.system(dense_fusion_cmd)
        if exit_code != 0:
