...
class GaussianModel:
    ...
    def get_rotation(self):
        # print("=" * 60)
        # print("DEBUG: self._rotation detailed information")
        # print("=" * 60)
        #
        # # 基本信息
        # print(f"Type: {type(self._rotation)}")
        # print(f"Shape: {self._rotation.shape}")
        # print(f"Dtype: {self._rotation.dtype}")
        # print(f"Requires grad: {self._rotation.requires_grad}")
        # print(f"Is leaf: {self._rotation.is_leaf}")
        #
        # # 维度信息
        # print(f"Number of dimensions: {self._rotation.dim()}") # 1
        # print(f"Number of elements: {self._rotation.numel()}") # 0
        #
        # # 数值统计
        # print(f"Min value: {self._rotation.min().item()}")
        # print(f"Max value: {self._rotation.max().item()}")
        # print(f"Mean value: {self._rotation.mean().item()}")
        # print(f"Std value: {self._rotation.std().item()}")
        #
        # # 检查维度是否正确
        # # if len(self._rotation.shape) == 1:
        # #     print("Warning: _rotation is 1D, reshaping...")
        # #     # 如果是1D，需要reshape为2D
        # #     num_points = self._rotation.shape[0] // 4  # 四元数有4个分量
        # #     self._rotation = self._rotation.view(num_points, 4)
        # #     print(f"Reshaped to: {self._rotation.shape}")
        #
        # print("=" * 50)
        # exit()
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        #colors = pcd.colors
        # print("=== DEBUG: Point Cloud Colors in create_from_pcd ===")
        # # 打印前几个点的颜色值
        # print(f"First 5 points colors:")
        # print(colors[:5])
        #
        # # 打印第一个点的详细颜色
        # if len(colors) > 0:
        #     print(f"First point RGB: {colors[0]}")
        #     print(f"First point - R: {colors[0][0]}, G: {colors[0][1]}, B: {colors[0][2]}")
        #     print(f"First point RGB: {colors[1]}")
        #     print(f"First point - R: {colors[1][0]}, G: {colors[1][1]}, B: {colors[1][2]}")
        #     print(f"First point RGB: {colors[2]}")
        #     print(f"First point - R: {colors[2][0]}, G: {colors[2][1]}, B: {colors[2][2]}")
        #     print(f"First point RGB: {colors[3]}")
        #     print(f"First point - R: {colors[3][0]}, G: {colors[3][1]}, B: {colors[3][2]}")
        #     print(f"First point RGB: {colors[4]}")
        #     print(f"First point - R: {colors[4][0]}, G: {colors[4][1]}, B: {colors[4][2]}")
        #
        # print("=" * 50)
    ...
    def update_with_pcd(self, pcd: o3d.geometry.PointCloud, merge_threshold=0.01, color_noise=0.05):
        """
        增量式添加新点云到高斯模型
        Args:
            pcd: Open3D点云对象
            merge_threshold: 点距阈值，小于该值的点会被视为重复点
            color_noise: 颜色扰动强度
        """
        # 转换点云数据为Tensor
        new_xyz = torch.tensor(np.asarray(pcd.points),dtype=torch.float32,device=self._xyz.device)

        # 如果有颜色数据，转换为SH系数
        if pcd.has_colors():
            new_colors = torch.tensor(np.asarray(pcd.colors),dtype=torch.float32,device=self._xyz.device)
            # 添加颜色噪声避免artifacts
            new_colors += (torch.rand_like(new_colors) * color_noise - color_noise / 2)
            new_features_dc = RGB2SH(new_colors)
        else:
            # 若无颜色，使用随机颜色
            new_features_dc = RGB2SH(torch.rand(new_xyz.shape[0], 3, device=new_xyz.device))

            # 去重逻辑
            if len(self._xyz) > 0:
                dists = torch.cdist(new_xyz, self._xyz)
            min_dists = dists.min(dim=1).values
            mask = min_dists > merge_threshold
        ...
        # 初始化新高斯参数
        new_features = torch.zeros((new_xyz.shape[0], 3, (self.max_sh_degree + 1) ** 2),device=new_xyz.device)
        new_features[:, :3, 0] = new_features_dc
        new_scaling = torch.log(torch.ones_like(new_xyz) * 0.01)  # 初始尺度
        new_rotation = torch.zeros((new_xyz.shape[0], 4), device=new_xyz.device)
        new_rotation[:, 0] = 1  # 单位四元数
        new_opacity = inverse_sigmoid(0.1 * torch.ones((new_xyz.shape[0], 1),device=new_xyz.device))
        new_knn_f = torch.randn((new_xyz.shape[0], 6), device=new_xyz.device)

        # 合并到现有模型
        self.densification_postfix(new_xyz,new_knn_f,new_features[:, :, 0:1].transpose(1, 2).contiguous(),new_features[:, :, 1:].transpose(1, 2).contiguous(),new_opacity,new_scaling,new_rotation)
