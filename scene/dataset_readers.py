...
from scene.pc_registrate import match_point_clouds
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    dense_pc_path = os.path.join(path, "dense/xuan_withcolor_merged_Cloud.ply")
    ...
    target = o3d.io.read_point_cloud(dense_pc_path)

    # 粗配准（可选：基于特征）
    trans_init = np.eye(4)  # 或用RANSAC得到
    ...
    # o3d.io.write_point_cloud("source_aligned.ply", source)
    results = match_point_clouds(source_aligned, target, path)
    pcd = o3d.io.read_point_cloud(results['output_file'])
