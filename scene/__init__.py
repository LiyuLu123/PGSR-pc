...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        """
        self.model_path = args.model_path # ./output/20-1
        self.loaded_iter = None
        self.gaussians = gaussians
        self.source_path = args.source_path #/home/share_data/20/scene
        # self.pcd_dir = os.path.join(args.source_path) #/home/share_data/20/scene/train_pcds
        ...
        # if os.path.exists(self.pcd_dir):  # 检查点云目录是否存在 # 从第一个训练相机的点云初始化
        #     train_cams = self.getTrainCameras()
        #     first_cam = train_cams[0]
        #     pcd_path = os.path.join(self.pcd_dir, f"{os.path.splitext(first_cam.image_name)[0]}.pcd")
        #     print(pcd_path)
        #     exit()
        #     if os.path.exists(pcd_path):
        #         init_pcd = o3d.io.read_point_cloud(pcd_path)
        #         self.gaussians.create_from_pcd(init_pcd)
        #     else:
        #         print(f"Warning: No PCD file found at {pcd_path}, using original point cloud")
    ...
    def getTrainCameras(self, scale=1.0):
        ...
        #     pcd_path = os.path.join(self.pcd_dir, f"{os.path.splitext(cam.image_name)[0]}.pcd")
        #     if os.path.exists(pcd_path):
        #         cam.pcd_path = pcd_path  # 动态添加属性
